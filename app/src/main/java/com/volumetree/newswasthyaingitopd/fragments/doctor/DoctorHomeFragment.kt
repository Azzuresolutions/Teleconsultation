...
import com.volumetree.newswasthyaingitopd.databinding.FragmentDoctorHomeBinding
...
class DoctorHomeFragment : Fragment(), View.OnClickListener,
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.switchOnline.setOnCheckedChangeListener(this)
        tfRegular = Typeface.createFromAsset(requireActivity().assets, "lexend_regular.ttf")
        profileViewModel.getDoctorProfile().observeOnce(viewLifecycleOwner) { profileResponse ->
            if (profileResponse.success) {
                binding.tvDrName.text =
                    "${profileResponse.model.firstName.capitalizeLetter()} ${profileResponse.model.lastName.capitalizeLetter()}"
                binding.tvDoctorSpeciality.text = profileResponse.model.specializationName
                PrefUtils.setDoctorData(requireActivity(), profileResponse.model)
                PrefUtils.setLoginToken(requireActivity(), profileResponse.token)
                binding.imgDr.loadImageURL(
                    requireActivity(),
                    profileResponse.model.imagePath,
                    isNoCache = true,
                    doctor = true
                )
    ...
    private fun getDashboardData() {
        ...
        masterViewModel.getConsultationsDashboard(2).observeOnce(viewLifecycleOwner) {
    ...
    private fun getAvailabilityStatus() {
        doctorViewModel.checkAvailability()
            .observeOnce(viewLifecycleOwner) { availabilityResponse ->
                if (availabilityResponse.success) {
                    manageAvailabilityStatus(availabilityResponse)
                }
            }
    }

    private fun manageAvailabilityStatus(availabilityResponse: AvailabilityResponse) {
        binding.switchOnline.setOnCheckedChangeListener(null)
        if (availabilityResponse.statusId == 1) {
            binding.switchOnline.isChecked = true

            binding.tvOnlineStatement.visibility = View.VISIBLE
            binding.tvDoctorAvailabilityStatus.text = getString(R.string.your_online)
            binding.tvOnlineStatement2.visibility = View.GONE
            binding.layDoctorHome.background =
                ContextCompat.getDrawable(requireActivity(), R.drawable.bottom_corner_rounded)
            binding.tvSubStatement.visibility = View.GONE

        } else if (availabilityResponse.statusId == 2) {
            binding.switchOnline.isChecked = false
            binding.tvDoctorAvailabilityStatus.text = getString(R.string.offline)
            binding.tvOnlineStatement.visibility = View.GONE
            binding.tvOnlineStatement2.visibility = View.VISIBLE
            binding.layDoctorHome.background =
                ContextCompat.getDrawable(
                    requireActivity(),
                    R.drawable.bottom_corner_offline_rounded
                )
            binding.tvSubStatement.visibility = View.VISIBLE
        }
        binding.switchOnline.setOnCheckedChangeListener(this)
    }
    ...
    override fun onCheckedChanged(compoundButton: CompoundButton, isChecked: Boolean) {
        doctorViewModel.changeAvailability(
            if (isChecked) {
                1
            } else {
                2
    ...
    private fun setupConsultationLineChart() {
        binding.rgFilter.setOnCheckedChangeListener { radioGroup, id ->
            when (id) {
                R.id.rgWeek -> {
                    masterViewModel.getConsultationsDayDashboard(1, 2)
                        .observeOnce(viewLifecycleOwner) {
                            manageDayWiseReports(it)
                        }
                }
                R.id.rgMonthly -> {
                    masterViewModel.getConsultationsDayDashboard(2, 2)
                        .observeOnce(viewLifecycleOwner) {
                            manageDayWiseReports(it)
                        }
                }
                R.id.rgYearly -> {
                    masterViewModel.getConsultationsDayDashboard(3, 2)
                        .observeOnce(viewLifecycleOwner) {
                            manageDayWiseReports(it)
                        }
                }
    ...
    private fun setupCharts() {
        setupConsultationLineChart()
        binding.rgWeek.isChecked = true
        masterViewModel.getConsultationsVC(1, 2).observeOnce(viewLifecycleOwner) {
            if (it.success && it.lstModel.size > 0) {
        ...
        masterViewModel.getConsultationsGenderDashboard(2).observeOnce(viewLifecycleOwner) {
